...
const Payment = () => {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      ...
      try {
        const res = await axios.get(`http://69.62.74.167:5000/api/cart/${userId}`);
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axios.put(
        `http://69.62.74.167:5000/api/orders/${orderId}/pay`,
        formData,
        {
          headers: {
  ...
  <img
    src={`http://69.62.74.167:5000/uploads/qr.png`} // ✅ Assuming vendor server runs on port 5001
    alt="Admin QR Code"
