    ...
    try {
      ...
      for (const order of allOrders) {
        ...
        for (const item of order.items) {
          ...
          // ⚠ Fix: Skip if no product or vendorId
          if (!product || !product.vendorId) continue;
  
          if (product.vendorId.toString() === vendorId.toString()) {
            vendorItems.push({
              productId: item.productId,orderNumber: order.orderNumber,
        ...
        // ⚠ Return only if vendor has at least one item in this order
...
router.put('/api/vendor/:orderId/status', authenticateVendor, async (req, res) => {
