...
const Orders: React.FC = () => {
  ...
  const fetchOrders = async () => {
    try {
      const token = localStorage.getItem('authToken');
      const res = await axios.get<ApiResponse>('https://vendor.peghouse.in/api/orders/orders', {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const updateOrderStatus = async (
    ...
    try {
      ...
      if (shouldRedirectToPickup) {
        ...
        axios.put(
          `https://vendor.peghouse.in/api/orders/orders/${orderId}/status`,
          { productId, status },
          { headers: { Authorization: `Bearer ${token}` } }
        ).then(() => {
          return axios.put(
            `https://vendor.peghouse.in/api/orders/orders/${orderId}/ready-for-pickup`,
            { 
              orderId: orderId, 
  ...
  const handleOrderAccept = async (order: Order) => {
    ...
    try {
      ...
      const updatePromises = order.items
        ...
        .map(item => 
          axios.put(
            `https://vendor.peghouse.in/api/orders/orders/${order.id}/status`,
      ...
      Promise.all(updatePromises).then(() => {
        return axios.put(
          `https://vendor.peghouse.in/api/orders/orders/${order.id}/ready-for-pickup`,
          { 
            orderId: order.id, 
            orderNumber: order.orderNumber,
            status: 'accepted'
  ...
  const handleOrderHandover = async (order: Order) => {
    ...
    try {
      ...
      try {
        await axios.post(
          'https://vendor.peghouse.in/api/orders/payouts/track',
          {
            orderId: order.id,
            orderNumber: order.orderNumber,
