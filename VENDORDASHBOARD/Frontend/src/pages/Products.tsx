  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const response = await axios.get('https://vendor.drnkly.in/api/products/vendor', {
          headers: {
    ...
    const fetchCategories = async () => {
      try {
        const response = await axios.get('https://vendor.drnkly.in/api/categories');
...
const handleAddProduct = async (e: React.FormEvent) => {
  ...
  try {
    const response = await axios.post('https://vendor.drnkly.in/api/products/add', formData, {
      headers: {
        'Authorization': `Bearer ${token}`,
  ...
  const handleEditProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://vendor.drnkly.in/api/products/${updatedProduct._id}`, // Use _id instead of id
        updatedProduct,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  const handleDeleteProduct = async (productId: string) => {
    if (window.confirm('Are you sure you want to delete this product?')) {
      try {
        ...
        await axios.delete(`https://vendor.drnkly.in/api/products/${productId}`, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
        ...
        {filteredProducts.map((product) => (
          <div key={product._id} className="bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-md transition-shadow">
            <div className="relative">
              <img
                src={`https://vendor.drnkly.in${product.image}`} // The image path returned by the backend
                alt={product.name}
